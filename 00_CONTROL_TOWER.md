...
**🏆 GOLD CHECKPOINTED · 89.3% · READY FOR INTERVIEW DEFENSE — post-GP5 + Opus calibration audit applied (docs corrected; champion unchanged). Changes NOT yet git-committed — see SESSION_STATE.md.**

**⚠ CALIBRATION CORRECTION (Opus audit, 2026-06-23) — READ THIS:** The SERVED calibrator is **isotonic** (`app.py` uses `iso_x.npy`/`iso_y.npy` via numpy interp). Audit-recomputed test **ECE = 0.0019**. The frozen GP2 single-shot test report scored the **Platt** variant (**ECE = 0.0034**) — that 0.0034 is Platt's number, not isotonic's. The earlier "`champion_calibrated.pkl` `cal['selected']='isotonic'`" forensics story was **FALSE** (that pkl is the unrelated G4 XGBoost demo) and has been removed from all candidate-facing docs. Champion AUC=0.7769 (raw/Platt; isotonic 0.7764 due to interp ties). All candidate-facing docs (defense MD+PDF, model card, resume pack, gold checkpoint, README, claim boundary) now state this side-by-side. Frozen `outputs/evidence/` artifacts unchanged (`gold_pass2_final_untouched_test_report.json` still labels Platt 0.0034 — correct for that artifact). 0.0019 is audit-recomputed, NOT in the frozen JSONs.

**Opus Audit Summary (2026-06-23):** 8-pass high-effort audit. Reproduced champion metrics exactly (AUC 0.776858, raw ECE 0.294812; Platt test ECE 0.003396; isotonic test ECE 0.001938). Fixes A–G applied: calibration story standardized (side-by-side), false pkl forensics removed, defense-doc self-contradiction reconciled, stale CatBoost-champion answers superseded, deployment narrative reconciled (committed `app.py` serves champion via native booster + numpy isotonic), imputation-leak documented, hardcoded `/sessions/...` paths fixed in `g9a_feature_engineering.py` / `g9a_model_tournament.py` / `g9a_fe_runner.sh`, build-breaking `import pandas as pd as pd_tmp` syntax bug removed from `g9a_model_tournament.py`, `docs/REPRODUCIBILITY_NOTE.md` added. Full detail: `OPUS_AUDIT_FINDINGS.md` (§8 resolution log). Open threads + commit command: `SESSION_STATE.md`.

**Gold Pass 5 Summary (LSTM challenger — negative result, documented):** 1-layer LSTM on 13.6M instalment rows → 32-dim embeddings → 172-feature LightGBM challenger. AUC=0.7264 vs GP2 champion 0.7769 (Δ=−0.0505). **Champion RETAINED.** Root cause: 35% zero-history applicants → padded-noise embeddings; scalar aggregates (INST_LATE_RATIO) already capture the signal. Files: `notebooks/GP5_LSTM_encoder.ipynb`, `scripts/gp5_preprocess_sequences.py`, `scripts/gp5_augment_and_retrain.py`, `outputs/models/lgb_gp5_172_challenger.txt`, `lstm_encoder.pt`, `gp5_embeddings_{train,val,test}.npy`.
...
**PROJECT STATUS: GOLD CHECKPOINTED · GP5 done (negative, champion retained) · Opus calibration audit applied 2026-06-23 (docs corrected, champion unchanged) · changes NOT yet git-committed — read SESSION_STATE.md first on resume.**
...
| GOLD_PASS_5 | ✓ COMPLETE (negative result) | notebooks/GP5_LSTM_encoder.ipynb · scripts/gp5_preprocess_sequences.py · scripts/gp5_augment_and_retrain.py · outputs/models/lgb_gp5_172_challenger.txt · lstm_encoder.pt · gp5_embeddings_{train,val,test}.npy | 1-layer LSTM (32-dim embeddings) → 172-feature LightGBM challenger | Challenger AUC=0.7264 vs champion 0.7769 (Δ=−0.0505) · CHAMPION RETAINED · root cause: 35% zero-history → padded-noise embeddings |
| OPUS_AUDIT | ✓ COMPLETE (2026-06-23) | OPUS_AUDIT_FINDINGS.md · OPUS_AUDIT_HANDOFF.md · docs/REPRODUCIBILITY_NOTE.md · SESSION_STATE.md · (edits: defense MD+PDF, model card, resume pack, gold checkpoint, README, claim boundary, g9a_* scripts) | No retraining — audit + doc/code corrections only | Reproduced AUC=0.776858 exactly · served isotonic test ECE=0.0019 / Platt frozen 0.0034 · false pkl forensics removed · hardcoded paths + tournament syntax bug fixed · NOT git-committed yet |
